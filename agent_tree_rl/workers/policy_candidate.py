...
from agent_tree_rl.artifacts import read_verified_policy  # noqa: E402
...
def main() -> int:
    ...
    artifact = Path(sys.argv[1]).expanduser()
    if not artifact.is_absolute():
        raise SystemExit("POLICY_ARTIFACT must be an absolute content-addressed path")
    model_payload = json.loads(read_verified_policy(artifact, artifact.stem))
